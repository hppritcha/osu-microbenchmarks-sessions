...
int main(int argc, char *argv[])
{
    ...
    */
        ...
        if(myid == 0) {
            for(i = 0; i < options.iterations + options.skip; i++) {
                ...
                for(j = 0; j < window_size; j++) {
                    MPI_CHECK(MPI_Irecv(r_buf, size, MPI_CHAR, 1, 10, lib_comm,
                            recv_request + j));
                }

                for(j = 0; j < window_size; j++) {
                    MPI_CHECK(MPI_Isend(s_buf, size, MPI_CHAR, 1, 100, lib_comm,
                            send_request + j));
                }
            ...
            for(i = 0; i < options.iterations + options.skip; i++) {
                for(j = 0; j < window_size; j++) {
                    MPI_CHECK(MPI_Irecv(r_buf, size, MPI_CHAR, 0, 100, lib_comm,
                            recv_request + j));
                }

                for (j = 0; j < window_size; j++) {
                    MPI_CHECK(MPI_Isend(s_buf, size, MPI_CHAR, 0, 10, lib_comm,
                            send_request + j));
                }
    ...
    MPI_CHECK(MPI_Comm_free(&lib_comm));
    MPI_CHECK(MPI_Group_free(&wgroup));
    MPI_CHECK(MPI_Session_finalize(&lib_shandle));
