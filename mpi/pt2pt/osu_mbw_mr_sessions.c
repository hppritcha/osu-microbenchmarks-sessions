...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    MPI_CHECK(MPI_Comm_rank(lib_comm, &rank));
        ...
        char *r_buf)
{
    ...
    if(rank < num_pairs) {
        ...
        for(i = 0; i <  options.iterations +  options.skip; i++) {
            ...
            for(j = 0; j < window_size; j++) {
                MPI_CHECK(MPI_Isend(s_buf, size, MPI_CHAR, target, 100, lib_comm,
                        mbw_request + j));
            }
            MPI_CHECK(MPI_Waitall(window_size, mbw_request, mbw_reqstat));
            MPI_CHECK(MPI_Recv(r_buf, 4, MPI_CHAR, target, 101, lib_comm,
                    &mbw_reqstat[0]));
        }
        ...
        for(i = 0; i <  options.iterations +  options.skip; i++) {
            ...
            for(j = 0; j < window_size; j++) {
                MPI_CHECK(MPI_Irecv(r_buf, size, MPI_CHAR, target, 100, lib_comm,
                        mbw_request + j));
            }

            MPI_CHECK(MPI_Waitall(window_size, mbw_request, mbw_reqstat));
            MPI_CHECK(MPI_Send(s_buf, 4, MPI_CHAR, target, 101, lib_comm));
        }
    }
    ...
    MPI_CHECK(MPI_Reduce(&t, &sum_time, 1, MPI_DOUBLE, MPI_SUM, 0, lib_comm));
